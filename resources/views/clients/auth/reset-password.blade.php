@extends('clients.layouts.master-auth')
@section('title')
<title>THAY ĐỔI MẬT KHẨU </title>

@endsection
@section('content')
<div class="auth-page-wrapper pt-5">
    <!-- auth page bg -->
    <div class="auth-one-bg-position auth-one-bg"  id="auth-particles">
        <div class="bg-overlay"></div>

        <div class="shape">
            <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 1440 120">
                <path d="M 0,36 C 144,53.6 432,123.2 720,124 C 1008,124.8 1296,56.8 1440,40L1440 140L0 140z"></path>
            </svg>
        </div>
    </div>

    <!-- auth page content -->
    <div class="auth-page-content">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="text-center mt-sm-5 mb-4 text-white-50">
                        <div>
                            <a href="index" class="d-inline-block auth-logo">
                                <img src="{{ URL::asset('assets/images/logo-light.png')}}" alt="" height="20">
                            </a>
                        </div>
                        
                    </div>
                </div>
            </div>
            <!-- end row -->

            <div class="row justify-content-center">
                <div class="col-md-8 col-lg-6 col-xl-5">
                    <div class="card mt-4">

                        <div class="card-body p-4">
                            <div class="text-center mt-2">
                                <h5 class="text-primary">Đặt lại mật khẩu</h5>
                                <p class="text-muted">Bước cuối cùng để kiểm soát tài khoản của mình</p>
                            </div>
                            
                            <div class="p-2 mt-4">
                                <form action="{{route('rqResetPass')}}" method="POST">
                                    @csrf
                                    
                                    <div class="mb-3">
                                        <label for="username" class="form-label">Nhập Mật Khẩu</label>
                                        <input type="text" class="form-control @error('password') is-invalid @enderror" name="password" placeholder="Nhập Mật Khẩu" required>
                                        @error('password')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                        @enderror
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label" for="password-input">Nhập Lại Mật Khẩu</label>
                                        <div class="position-relative auth-pass-inputgroup mb-3">
                                            <input type="text" class="form-control pe-5 @error('repassword') is-invalid @enderror" name="repassword"  placeholder="Nhập Lại Mật Khẩu" required>
                        
                                            @error('repassword')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                    <input type="text" name="token" value="{{$token}}" hidden>
                                    </div>
                                    <div class="mt-4">
                                        <button class="btn btn-success w-100" type="submit" id="btnsubmit" name="submit">Thay Đổi</button>
                                    </div>

                                    
                                </form>
                            </div>
                        </div>
                        <!-- end card body -->
                    </div>
                    <!-- end card -->

                </div>
            </div>
            <!-- end row -->
        </div>
        <!-- end container -->
    </div>
    <!-- end auth page content -->

    <!-- footer -->
    <footer class="footer">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="text-center">
                        <p class="mb-0 text-muted">&copy; <script>document.write(new Date().getFullYear())</script>  DVMXH. Xây dựng và phát triển <i class="mdi mdi-heart text-danger"></i> by Yukiboyk</p>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <!-- end Footer -->
</div>
@endsection

@push('script')
<script src="{{url('assets/libs/particles.js/particles.js.min.js')}}"></script>
<script src="{{url('assets/js/pages/particles.app.js')}}"></script>
<script src="{{url('assets/js/pages/password-addon.init.js')}}"></script>
@endpush
